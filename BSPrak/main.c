...
int main() {
    ...
    while (1) {
        ...
        printf("Expecting commands");
        bytes_read = (int)read(filedescriptor, in, 1024);
        printf("Expecting command");

        while (bytes_read > 0) {
            scanf("%c", input);
            if (correctForm(input)) {
                switch (whichCommand(input)) {
                    case 0:
                        printf("No valid command found");
                    case 1:
                        put(extractKeyName(input), extractValue(input));
                    case 2:
                        get(extractKeyName(input));
                    case 3:
                        del(extractKeyName(input));
                }
            }
